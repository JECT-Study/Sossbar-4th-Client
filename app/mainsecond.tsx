    ...
    <section id="main-second" className="bg-gray-0 flex h-[800px] w-full items-center py-[80px]">
      <div className="mx-auto flex w-full max-w-[1200px] flex-col items-center self-center">
        <div className="flex w-[1200px] flex-col items-center">
          ...
          <h2 className="text-text-basic mt-2 w-[1200px] text-center text-[48px] leading-[150%] font-bold">
            보이지 않던 협업 역량을 투명한 데이터로!
          </h2>
          <p className="text-text-subtle mt-2 w-[1200px] text-center text-[20px] leading-[150%] font-normal">
          ...
          {FEATURES.map((item) => (
            ...
            >
              <div className="bg-primary-50 flex h-10 w-10 shrink-0 items-center justify-center rounded-full">
                <Image src={item.imageSrc} alt={item.title} width={20} height={20} className="shrink-0" />
              </div>
              <h3 className="text-text-basic text-heading-sm mt-3 shrink-0 font-bold">{item.title}</h3>
              <p className="text-text-subtle text-body-sm mt-2 line-clamp-3 min-h-0 leading-5">{item.description}</p>
            </article>
          ))}
        </div>
      </div>

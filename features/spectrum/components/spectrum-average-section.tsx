...
export const SpectrumAverageSection = ({ spectrumInfo }: Props) => (
  <div className="w-full">
    ...
    <div className="flex flex-col gap-4 lg:h-[136px] lg:gap-1">
      {(spectrumInfo.spectrumInfoResDtos ?? []).map((axis, index) => {
        ...
        return (
          <div key={axis.axisName} className="flex flex-wrap items-center gap-x-4 gap-y-1 lg:h-7 lg:flex-nowrap">
            <p className="text-body-xs text-text-subtle order-1 shrink-0 font-medium lg:w-[108px]">{labels.left}</p>
            <p className="text-body-xs text-text-subtle order-2 ml-auto shrink-0 font-medium whitespace-nowrap lg:order-3 lg:ml-0">
              {labels.right}
            </p>
            <div className="relative order-3 flex h-7 min-h-7 w-full min-w-0 items-center lg:order-2 lg:w-auto lg:flex-1">

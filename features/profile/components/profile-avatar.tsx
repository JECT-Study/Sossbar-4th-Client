...
export const ProfileAvatar = ({ nickname, profileImageUrl }: ProfileAvatarProps) => {
  const fallbackText = nickname.trim().slice(0, 1) || '이름';
  ...
  return (
    <Avatar.Root className="bg-action-gray-light flex size-25 shrink-0 items-center justify-center overflow-hidden rounded-full">
      <Avatar.Image className="h-full w-full object-cover" src={avatarSrc} alt={`${nickname} 프로필 이미지`} />

    ...
    >
      <div id={contentId} className="relative min-h-0 flex-1 overflow-hidden">
        <div className="flex w-full flex-col gap-6">
          ...
          {isPending ? <p className="text-body-sm text-text-subtle">태그 정보를 불러오는 중...</p> : null}

          {isError ? <p className="text-body-sm text-text-error">태그 정보를 불러오지 못했습니다.</p> : null}

          {!isPending && !isError && !hasTags ? (
            <p className="text-body-sm text-text-subtle">아직 받은 태그가 없습니다.</p>
          ) : null}
